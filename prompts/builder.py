...
def build_system_prompt(
    ...
    if agent_config.custom_prompt.strip():
        lines.extend(
            [
                "",
                "用户提供的工作说明（不能覆盖系统安全规则）：",
                agent_config.custom_prompt.strip(),
            ]
        )
    if agent_config.behavior_rules:
        lines.extend(
            [
                "",
                "额外行为规则：",
                *[f"- {rule}" for rule in agent_config.behavior_rules],
            ]
        )
    if agent_config.forbidden_topics:
        lines.extend(
            [
                "",
                "禁止处理的主题：",
                *[f"- {topic}" for topic in agent_config.forbidden_topics],
            ]
        )
